...
class PW_WeiboBindService {
	...
	function getAppConfigUrl() {
		$platformApiClient = $this->_getPlatformApiClient();
		return $platformApiClient->buildPageUrl(PW_PLATFORM_CLIENT_DEFAULT_ADMIN_USER_ID, 'weibo.setting');
	}
	
	function getLoginUrl($loginSessionId) {
		$platformApiClient = $this->_getPlatformApiClient();
		return $platformApiClient->buildPageUrl(PW_PLATFORM_CLIENT_DEFAULT_GUEST_USER_ID, 'weibo.login.guide', array('siteSessionId' => $loginSessionId));
	}
	...
	function isLoginUserNotResetPassword($userId) {
		...
		return $loginUser && $loginUser['hasresetpwd'] == '0';
	}
	...
	function callPlatformUnBind($siteUserId, $weiboType) {
		$platformApiClient = $this->_getPlatformApiClient();
		$isSuccess = PlatformApiClientUtility::decodeJson($platformApiClient->post('weibo.bind.release', array('site_uid' => $siteUserId, 'weibo_type' => $weiboType)));
