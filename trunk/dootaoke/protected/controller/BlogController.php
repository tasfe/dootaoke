...
class BlogController extends DooController {
	...
	function home() {
        ...
        $pager = new DooPager(Doo::conf()->APP_URL.'blog/page', $p->count(), 10, 10);
	...
	function getArticle() {
        ...
        $this->renderc('blog_single', $this->data);
	}
	...
	function getTag() {
        ...
        $totalPosts = $tag->relatePost(array('select'=>'COUNT(tag.id) AS total','groupby'=>'tag.id', 'asArray'=>true));
    ...
    function newComment(){
        ...
        if($error = $c->validate( 'skip' ) ){
            ...
            $this->renderc('blog_error', $this->data);
        }
        else{
            ...
            $this->data['rootUrl'] = Doo::conf()->APP_URL;
            $this->renderc('blog_comment', $this->data);
        }
    }
